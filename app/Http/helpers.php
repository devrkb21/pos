 ...
 * boots pos.
 */
function pos_boot( $ul, $pt, $lc, $pid = null, $app_ver = null ){
    
    $pid        = !empty( $pid ) ? $pid : config('author.pid');
    $app_ver    = !empty( $pid ) ? $app_ver : config('author.app_version');
    
    $ch         = curl_init();
    $curlConfig = [
        CURLOPT_URL     => base64_decode(config('author.lic1')),
        CURLOPT_POST    => true,
    ...
    if ($result) {
        ...
        if ($result['flag'] == 'valid') {
            return [
                'install_type'  => (!empty( $result['install_type'] )) ? ((int)$result['install_type']) : 2,
                'boot_type'     => (isset( $result['boot_type'] )) ? ((int)$result['boot_type']) : 0
            ];
        } 
        else {
